...
const Upload = () => {
  const { removeBg } = useContext(AuthContext)
  ...
  const handleUploadClick = (e) => {
    ...
    if (!token) {
      e.preventDefault(); // Prevent file upload
  ...
  return (
    <div className='pb-16'>
      ...
      <div className='text-center mb-24'>
        <input onChange={e=>removeBg(e.target.files[0])} type="file" accept='image/start' id="upload2" hidden onClick={handleUploadClick} />
        <label 
